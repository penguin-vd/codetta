...
import { DOCS, DOC_KINDS, docBySlug, type DocEntry } from "../docs.ts";
    ...
    <div className="fixed inset-0 z-50 grid grid-rows-[auto_1fr] bg-bg text-cream">
      <header className="flex items-center justify-between border-b border-line px-6 py-3">
        <div className="flex items-baseline gap-3">
          ...
          <span className="font-mono text-[10px] uppercase tracking-[0.25em] text-dim">language reference</span>
        </div>
        <a
          href="#/"
    ...
    <nav className="min-h-0 overflow-auto border-r border-line px-3 py-5">
      ...
      {DOC_KINDS.map((kind) => (
        <div key={kind} className="mt-5">
          <div className="px-3 pb-1.5 font-mono text-[10px] uppercase tracking-[0.2em] text-dim/70">{kind}s</div>
          {DOCS.filter((d) => d.kind === kind).map((d) => (
            <a
              key={d.slug}
              href={docsHref(d.slug)}
              className={`block rounded px-3 py-1.5 font-mono text-xs transition ${
                d.slug === activeSlug ? "bg-raise text-gold" : "text-dim hover:text-cream"
              }`}
            >
              {d.title}
            </a>
  ...
  return (
    <div className="mx-auto max-w-2xl px-10 py-10">
      ...
      <p className="mt-3 leading-relaxed text-dim">
        Codetta is a small language for writing music as code. Define chords and phrases, arrange them into
        sections and a song, then play or export to MIDI. Pick a topic, or hover a keyword in the editor and
        ...
        <section key={kind} className="mt-9">
          ...
          <div className="mt-3 grid gap-2">
            {DOCS.filter((d) => d.kind === kind).map((d) => (
              ...
              >
                <div className="font-mono text-sm text-cream group-hover:text-gold">{d.title}</div>
...
function Article({ entry }: { entry: DocEntry }) {
  return (
    <article className="mx-auto max-w-2xl px-10 py-10">
      <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-dim">{entry.kind}</div>
