...
export function TrackRail({ tracks, instruments, muted, soloed, onInstrument, onToggleMute, onToggleSolo }: Props) {
  return (
      ...
      {tracks.map((track, i) => (
        ...
        >
          ...
          <div className="flex min-w-0 flex-col">
            <span className="truncate font-serif text-[15px] font-medium text-cream">{track.name}</span>
            <span className="font-mono text-[10px] text-dim">{track.notes.length} notes</span>
          </div>
          ...
          <div className="flex gap-1">
            <Toggle active={muted.has(track.name)} activeClass="bg-coral text-[#1a0c0e]" onClick={() => onToggleMute(track.name)}>
              M
            </Toggle>
            <Toggle active={soloed.has(track.name)} activeClass="bg-gold text-[#2a1c06]" onClick={() => onToggleSolo(track.name)}>
              S
            </Toggle>
          </div>
        </div>
  ...
  return (
    <button
      ...
      className={`h-[26px] w-[26px] rounded-md border font-mono text-[11px] font-bold transition ${
        active ? `${activeClass} border-transparent` : "border-line bg-raise text-dim hover:text-cream"
      }`}
    >
      {children}
    </button>
