...
export function PianoRoll({ song, engine, playing, audible, dark }: Props) {
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: reset view when song changes
    useEffect(() => {
        scrollRef.current = 0;
        zoomRef.current = 1;
        setZoom(1);
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: redraw on zoom change
    useEffect(() => {
    ...
    const onPointerMove = (e: React.PointerEvent) => {
        ...
        if (!song || !layoutRef.current || !canvasRef.current) {
            setTooltip(null);
            return;
        }
        ...
        for (const track of song.tracks) {
            for (const n of track.notes) {
                ...
                if (tickAt >= n.ticks && tickAt < n.ticks + n.durationTicks) {
                    hit = {
                        note: noteName(n.midi),
                        vel: `${velLabel(n.velocity)} (${Math.round(n.velocity * 127)})`,
                    };
                    break;
                }
            }
        ...
        <div className="group relative h-full min-h-0 overflow-hidden">
            ...
            {song && (
                <div className="absolute right-3 top-3 flex items-center gap-1 rounded-lg border border-line bg-panel/80 px-1 py-1 opacity-0 backdrop-blur transition-opacity duration-200 group-hover:opacity-100">
                    ...
                    <span className="w-10 text-center font-mono text-[10px] text-dim tabular-nums">
                        {Math.round(zoom * 100)}%
                    </span>
                    <ZoomBtn
                        label="+"
